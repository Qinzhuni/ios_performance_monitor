...
- (void)addWithClsCallModel:(SMCallTraceTimeCostModel *)model {
    ...
    [self.dbQueue inDatabase:^(FMDatabase *db){
        if ([db open]) {
            //添加白名单
            FMResultSet *rsl = [db executeQuery:@"select cid,frequency,timecost from clscall where path = ?", model.path];
            if ([rsl next]) {
                //有相同路径就更新路径访问频率
                int fq = [rsl intForColumn:@"frequency"] + 1;
                double timecost = [rsl doubleForColumn:@"timecost"] + model.timeCost;
                int cid = [rsl intForColumn:@"cid"];
                [db executeUpdate:@"update clscall set frequency = ?, timecost = ? where cid = ?", @(fq), @(timecost), @(cid)];
            } else {
                //没有就添加一条记录
                NSNumber *lastCall = @0;
                if (model.lastCall) {
...
- (RACSignal *)selectClsCallWithPage:(NSUInteger)page {
    ...
    return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
        ...
        FMDatabase *db = [FMDatabase databaseWithPath:self.clsCallDBPath];
        if ([db open]) {
            FMResultSet *rs = [db executeQuery:@"select * from clscall where lastcall=? order by timecost desc limit ?, 50",@1, @(page * 50)];
            NSUInteger count = 0;
            NSMutableArray *arr = [NSMutableArray array];
            while ([rs next]) {
