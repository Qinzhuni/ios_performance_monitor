...
+ (void)save {
    ...
    for (SMCallTraceTimeCostModel *model in arr) {
        //记录方法路径
        model.path = [NSString stringWithFormat:@"0.[%@ %@] cost:%.3fms",model.className,model.methodName,model.timeCost * 1000];
...
//    [mStr appendFormat:@"%@\n path%@\n",[cost des],cost.path];
    if (cost.subCosts.count < 1) {
        ...
        //找到最耗时的路径保存
        SMCallTraceTimeCostModel *mostCostModel;
        for (SMCallTraceTimeCostModel *model in cost.subCosts) {
            ...
            if (!mostCostModel) {
                mostCostModel = model;
            }else if (model.timeCost > mostCostModel.timeCost) {
                mostCostModel = model;
            }
        }
        //记录方法的子方法的路径
        mostCostModel.path = [NSString stringWithFormat:@"%@ - %zd.[%@ %@] cost:%.3fms",cost.path,mostCostModel.callDepth,mostCostModel.className,mostCostModel.methodName,mostCostModel.timeCost * 1000];
        mostCostModel.timeCost = cost.timeCost;
        [self appendRecord:mostCostModel to:mStr];
//        for (SMCallTraceTimeCostModel *model in cost.subCosts) {
//            if ([model.className isEqualToString:@"SMCallTrace"]) {
//                break;
//            }
