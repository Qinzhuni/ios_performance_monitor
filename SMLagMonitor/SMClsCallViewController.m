...
#import "LMBBaseTableView.h"

static NSString *smClsCallCellIdentifier = @"smClsCallCell";

@interface SMClsCallViewController ()<UITableViewDataSource,UITableViewDelegate,PullTableViewDelegate>

@property (nonatomic, strong) NSMutableArray *listData;
@property (nonatomic, strong) LMBBaseTableView *tbView;
...
//                self.tbView.mj_footer.hidden = YES;
        ...
        //处理无数据显示
        [self.tbView.footer noticeNoMoreData];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CGRect frame = [model.path boundingRectWithSize:CGSizeMake([UIScreen mainScreen].bounds.size.width - 20*2, 999) options:NSStringDrawingUsesFontLeading | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingTruncatesLastVisibleLine attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:14]} context:nil];
...
- (UITableView *)tbView {
    if (!_tbView) {
        _tbView = [[LMBBaseTableView alloc] initWithFrame:CGRectZero];
        _tbView.dataSource = self;
        _tbView.delegate = self;
        _tbView.pullDelegate = self;
        [_tbView removeHeader];
...
- (void)pullTableViewDidStartPullUpLoadMore:(LMBBaseTableView *)tableView {
    [self selectItems];
}
